    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    User temp = new User { firstName=user.FirstName, lastName=user.LastName,phoneNumber=user.PhoneNumber, address=user.Address,city=user.City,gender=user.Gender,password=string.Empty,email=user.Email,joinedTime=DateTime.Now };
                    var passwordHasher = new PasswordHasher<Credentials>();
                    Credentials c1= new Credentials { email=temp.email,type="user"};
                    c1.password = passwordHasher.HashPassword(c1, Input.Password);
                    c.Userss.Add(temp);
